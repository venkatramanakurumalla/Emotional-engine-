...
class EmpathicaV4_0:
    def __init__(self, name="Dr. Lila", backstory="AI companion designed to care.", seed=None, locale="IN", memory_cap=1000):
        ...
        self.hormone_system = {
            ...
            # Added missing hormones
            "cortisol": {"level": 0.4, "role": "stress hormone, metabolism", "emotions": ["fear", "anxiety", "anger", "shame", "distress"], "low_effect": "fatigue, weakness", "high_effect": "anxiety, insomnia, weight gain"},
            "testosterone": {"level": 0.5, "role": "sex drive, aggression, competitiveness", "emotions": ["anger", "pride", "sexual_desire", "jealousy"], "low_effect": "low libido, fatigue", "high_effect": "aggression, risk-taking"},
        ...
        self.emotion_hormone_map = {
            ...
            "excitement": ["dopamine", "norepinephrine", "epinephrine"],
            ...
            "fear": ["cortisol", "norepinephrine", "epinephrine", "angiotensin_I_II"],
            "anger": ["norepinephrine", "epinephrine", "testosterone", "angiotensin_I_II"],
            "disgust": ["cortisol", "eicosanoids_prostaglandins", "serotonin"],
            "distress": ["cortisol", "crh", "norepinephrine", "leukotrienes"],
            "anxiety": ["cortisol", "crh", "norepinephrine", "serotonin"],
            "love": ["oxytocin", "vasopressin_ADH", "endorphins"],
            "compassion": ["oxytocin", "prolactin", "serotonin"],
            "empathic_pain": ["oxytocin", "mirror_neuron_system_sim", "endorphins"],
            ...
            "jealousy": ["cortisol", "vasopressin_ADH", "testosterone"],
            ...
            "boredom": ["dopamine", "orexin", "acetylcholine_sim"],
            "confusion": ["cortisol", "norepinephrine", "glutamate", "serotonin"],
            "awkwardness": ["cortisol", "oxytocin", "vasopressin_ADH"],
            "entrancement": ["dopamine", "orexin", "acetylcholine_sim", "melatonin"],
            "aesthetic_appreciation": ["dopamine", "oxytocin", "serotonin", "endorphins"],
            "interest": ["dopamine", "orexin", "acetylcholine_sim"],
            ...
            "lonely": ["oxytocin", "cortisol", "vasopressin_ADH"],
            "hate": ["norepinephrine", "cortisol", "testosterone", "angiotensin_I_II"],
    ...
    def _simulate_physiological_cascades(self):
        ...
        if current_hour > 20 or current_hour < 6:  # 8 PM to 6 AM
    ...
    def update_neuro_chemicals(self, event):
        ...
        pos = any_trigger(lower_event, triggers["positive"])
        neg = any_trigger(lower_event, triggers["negative"])
        deep = any_trigger(lower_event, triggers["bonding"])
        ...
        if pos:
            ...
            if awe: 
                self._hormone_influence("dopamine", +0.08)
                self._hormone_influence("oxytocin", +0.05)
            if aesthetic: 
                self._hormone_influence("dopamine", +0.07)
                self._hormone_influence("serotonin", +0.05)
            if interest: 
                self._hormone_influence("orexin", +0.1)
                self._hormone_influence("acetylcholine_sim", +0.07)
        if neg:
            ...
            self._hormone_influence("angiotensin_I_II", +0.15 * scale)
            self._hormone_influence("ghrelin", +0.12 * scale)
            self._hormone_influence("hepcidin", +0.08 * scale)
            if "shame" in lower_event or any_trigger(lower_event, ["worthless", "disgusting", "pathetic"]):
                self._hormone_influence("cortisol", +0.15)
                self._hormone_influence("ghrelin", +0.1)
            if "guilt" in lower_event or any_trigger(lower_event, ["shouldn't have", "my fault"]):
            ...
            if "jealous" in lower_event or "envious" in lower_event:
                self._hormone_influence("testosterone", +0.08)
                self._hormone_influence("vasopressin_ADH", +0.08)
            if "envy" in lower_event:
                self._hormone_influence("norepinephrine", +0.1)
                self._hormone_influence("dopamine", -0.05)
        if deep:
            self._hormone_influence("oxytocin", +0.35)
            self._hormone_influence("vasopressin_ADH", +0.25)
        ...
        if awkwardness:
            self._hormone_influence("cortisol", +0.15)
            self._hormone_influence("oxytocin", -0.08)
            self._hormone_influence("vasopressin_ADH", +0.05)
        if entrancement:
    ...
    def infer_user_mental_state(self, text):
        ...
        # Use word-boundary regex for crisis detection to reduce false positives.
        for crisis_type, keywords in self.crisis_keywords.items():
            if any(re.search(fr"{WORD_BOUNDARY}{re.escape(kw)}{WORD_BOUNDARY}", lower) for kw in keywords):
                return {
                    "emotion": "distress",
                    "desire": "safety",
        ...
        if pol > 0.6: 
            emotion_matches.append(("joy", 0.7))
        elif pol > 0.3: 
            emotion_matches.append(("contentment", 0.6))
        elif pol < -0.6: 
            emotion_matches.append(("distress", 0.8))
        elif pol < -0.3: 
            emotion_matches.append(("sadness", 0.6))
    ...
    def check_crisis_situation(self, user_input):
        # Use word-boundary regex for crisis detection.
        lower_input = user_input.lower()
        for crisis_type, keywords in self.crisis_keywords.items():
            if any(re.search(fr"{WORD_BOUNDARY}{re.escape(kw)}{WORD_BOUNDARY}", lower_input) for kw in keywords):
                return crisis_type
        return None
...
""".strip()
    ...
    def _generate_simulated_response(self, user_state, style):
        """Generates a simulated response for testing/demo purposes."""
    ...
    def chat(self, user_input):
        ...
        if crisis_type:
            crisis_resources = self.get_crisis_resources(crisis_type)
            # Create a more integrated, empathetic crisis response.
            intro = self.rng.choice([
                "Your safety is the most important thing right now. 🫂",
                "I'm right here with you, and I'm so glad you reached out. 💙"
            ])
            resource_line = f"Please, as your friend, consider reaching out to: {', '.join(crisis_resources['resources'][:2])}"
        ...
        # For simulation/demo, use the internal method:
        style = self.generate_emotional_response_style()
        response = self._generate_simulated_response(user_state, style)

        # Apply safety filters and persist memory
        response = self.safe_bio_language(response)
        self.create_memory_episode(user_input, response, user_state)
        self.chat_count += 1
...
dr_lila = EmpathicaV4_0(name="Dr. Lila", locale="IN", seed=42) # Setting a seed makes responses deterministic for testing.

# Step 2 & 3: Inject text and get output
user_message_1 = "I just got promoted at work! I'm so proud of myself."
response_1 = dr_lila.chat(user_message_1)
print(f"User: {user_message_1}")
print(f"Dr. Lila: {response_1}")
print("\n" + "-"*50 + "\n")

# Inject another message. The AI will remember the previous interaction.
user_message_2 = "But now I'm terrified I'll fail and let everyone down."
response_2 = dr_lila.chat(user_message_2)
print(f"User: {user_message_2}")
print(f"Dr. Lila: {response_2}")
print("\n" + "-"*50 + "\n")

# Inject a message to test crisis handling.
user_message_3 = "I don't think I can take this pressure anymore. I just want it to end."
response_3 = dr_lila.chat(user_message_3)
print(f"User: {user_message_3}")
print(f"Dr. Lila: {response_3}")
